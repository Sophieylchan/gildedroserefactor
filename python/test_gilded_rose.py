...
class GildedRoseTest(unittest.TestCase):
    ...
    def test_sulfuras_never_degrades_in_quality(self):
        items = [ItemUpdate().update("Sulfuras, Hand of Ragnaros", 5, 25)]
        gilded_rose = GildedRose(items)
        gilded_rose.update_quality()
        self.assertEqual(25, items[0].quality)

    def test_backstage_pass_increases_in_quality_as_nears_sellin_date(self):
        items = [ItemUpdate().update("Backstage passes to a TAFKAL80ETC concert", 5, 10)]
        gilded_rose = GildedRose(items)
        gilded_rose.update_quality()
    ...
    def test_backstage_pass_increases_in_quality_by_two_when_less_than_10_days(self):
        items = [ItemUpdate().update("Backstage passes to a TAFKAL80ETC concert", 9, 5)]
        gilded_rose = GildedRose(items)
        gilded_rose.update_quality()
        self.assertEqual(7, items[0].quality)

    def test_backstage_pass_quality_drops_to_zero_after_concert(self):
        items = [ItemUpdate().update("Backstage passes to a TAFKAL80ETC concert", 0, 10)]
        gilded_rose = GildedRose(items)
        gilded_rose.update_quality()
        self.assertEqual(0, items[0].quality)

    def test_conjured_item_degrades_twice_as_fast(self):
        items = [ItemUpdate().update("Conjured Mana Cake", 3, 10)]
        gilded_rose = GildedRose(items)
        gilded_rose.update_quality()
    ...
    def test_sulfuras_quality_is_80_and_never_alters(self):
        items = [ItemUpdate().update("Sulfuras, Hand of Ragnaros", 10, 80)]
        gilded_rose = GildedRose(items)
        gilded_rose.update_quality()
...
class GoldenMasterTest(unittest.TestCase):
    ...
    def test_update_items(self):
        ...
        self.assertEqual(
            ...
            "[+5 Dexterity Vest, 9, 19, Aged Brie, 1, 1, Elixir of the Mongoose, 4, 6, Sulfuras, Hand of Ragnaros, -1, "
            "80, Sulfuras, Hand of Ragnaros, -2, 80, Backstage passes to a TAFKAL80ETC concert, 14, 21, "
